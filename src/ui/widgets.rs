...
pub(super) fn render_title(f: &mut Frame, app: &App, area: Rect) {
    ...
    let line = Line::from(vec![
        Span::styled(format!("  {} ", app.base_path), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
...
pub(super) fn render_repo_list(f: &mut Frame, app: &App, area: Rect) {
    let items: Vec<ListItem> = app.repos.iter().enumerate().map(|(i, r)| {
        ...
        if r.ahead > 0 || r.behind > 0 {
            spans.push(Span::styled(
                ...
                if selected { Style::default().fg(Color::Black).bg(color) } else { Style::default().fg(Color::Blue) },
            ));
        }
        if r.modified + r.staged + r.untracked > 0 {
            spans.push(Span::styled(
                format!("  +{} ~{} ?{}", r.staged, r.modified, r.untracked),
                if selected { Style::default().fg(Color::Black).bg(color) } else { Style::default().fg(Color::Yellow) },
            ));
        }
    ...
    f.render_stateful_widget(
        List::new(items).block(block("Repositories", true)).highlight_style(Style::default()),
        area,
        &mut state,
    );
}
...
pub(super) fn render_help(f: &mut Frame, area: Rect) {
    ...
    let help = Paragraph::new(vec![
        Line::from(vec![
            Span::styled("↑↓", c), Span::styled(" navigate  ", d),
            Span::styled("Enter", c), Span::styled("/", d),
            Span::styled("s", c), Span::styled(" details  ", d),
            Span::styled("S", c), Span::styled(" status all", d),
        ]),
        Line::from(vec![
            Span::styled("u", g), sl.clone(), Span::styled("U", g), Span::styled(" pull  ", d),
            Span::styled("f", g), sl.clone(), Span::styled("F", g), Span::styled(" fetch", d),
        ]),
        Line::from(vec![
            Span::styled("l", b), sl.clone(), Span::styled("L", b), Span::styled(" log  ", d),
            Span::styled("d", r), sl.clone(), Span::styled("D", r), Span::styled(" discard", d),
        ]),
        Line::from(vec![
            Span::styled("c", y), sl.clone(), Span::styled("C", y), Span::styled(" cmd  ", d),
            Span::styled("p", m), sl.clone(), Span::styled("P", m), Span::styled(" pr  ", d),
            Span::styled("n", m), Span::styled(" pr create", d),
        ]),
        Line::from(vec![
            Span::styled("a", y), sl.clone(), Span::styled("A", y), Span::styled(" ci runs  ", d),
            Span::styled("o", m), Span::styled(" open repo", d),
        ]),
        Line::from(vec![
            Span::styled("s", c), Span::styled("/", d),
            Span::styled("S", c), Span::styled(" status", d),
        ]),
        Line::from(vec![
            Span::styled("r", y), Span::styled(" refresh  ", d),
            Span::styled("h", d), Span::styled(" help  ", d),
            Span::styled("q", r), Span::styled(" quit", d),
        ]),
...
pub(super) fn render_status(f: &mut Frame, app: &App, area: Rect) {
    let (text, color) = if app.confirm_mode {
        ...
        let (mode_label, color) = if shell_mode { ("sh", Color::Magenta) } else { ("git", Color::Green) };
    ...
    let border_color = if app.confirm_mode {
        ...
        if app.input_buffer.starts_with('!') { Color::Magenta } else { Color::Green }
    } else {
        Color::DarkGray
    };
